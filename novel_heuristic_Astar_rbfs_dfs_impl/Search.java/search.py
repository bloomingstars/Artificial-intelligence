...
def old_RBFS_analysis(node, flimit, destn, heuristic, parent_node, path,expandedcities=set(),successorlength=0):
    ...
    expandedcities.add(node)
    print("in rbfs with node : ", node)
    if node == destn or time.time()-start> 5000:
        return node, -1, expandedcities, path,0
    print("out1")
    removelist = set()
    if parent_node is not None:
    ...
    while True:
        ...
        # print(successors)
        # print(successors[0])
        print(name_city_map[successors[0]].f)
        # print(successors.sort(key=lambda x: (name_city_map[x].f)))
        # successors.sort(key=lambda x: (name_city_map[x].f))  # Order by lowest f value
        min_v=name_city_map[successors[0]].f
        min_index=0
        print("out4.25")
        for i in range(0,len(successors)):
            print("out4.225")
            if(name_city_map[successors[i]].f < min_v):
                min_v=name_city_map[successors[i]].f
                min_index=i
        print("out4.35")
        tmp=successors[0]
        successors[0]=successors[min_index]
        successors[min_index]=tmp
        ...
        alternative_f = float('inf')
        if len(successors) > 1:
            min_v = name_city_map[successors[1]].f
            min_index = 1
            for i in range(1,len(successors)):
                if (name_city_map[successors[i]].f < min_v):
                    ...
                    min_index = i
            tmp = successors[1]
            successors[1] = successors[min_index]
            successors[min_index] = tmp
            alternative_f = name_city_map[successors[1]].f
        print("out7")
        if time.time() - start >3600:
            return node, -1, expandedcities, path, 0
        print("out9")
        print(best, min(flimit, alternative_f), destn, heuristic, node, path)
        ...
        for (a_name, a_city) in name_city_map.items():
            if name != a_name:
                ...
                try:
                    ...
                    continue
                print(" result of",name, a_name, " : ",x,y,expanded,path,qlength)
...
def callRBFSAnalysis(src, destn, heuristic):
    if heuristic == 0:
        ...
        result= old_RBFS_analysis(src, float('inf'), destn, False, None, [src],expandedcities=set())
        #result= new_RBFS_analysis(src, src, float('inf'), destn, False, None, [src], current_path=set(), expandedcities = set())
